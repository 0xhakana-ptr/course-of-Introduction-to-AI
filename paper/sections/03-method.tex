...
关键词组合门控的判定逻辑分为三个阶段：（1）空输入过滤，将空或纯空白输入直接判定为 $i_{\text{unknown}}$；（2）快速通道，若输入包含文件引用或代码结构特征则立即判定为 $i_{\text{coding}}$；（3）特征标志提取与组合判断，提取 8 个布尔特征标志后按优先级依次判定。

\begin{lstlisting}[caption={关键词组合门控伪代码}, label={lst:keyword-gate}, basicstyle=\small\ttfamily, frame=single, breaklines=true]
function keyword_gate(x) -> RoutingDecision:
    if is_blank(x):
        return RoutingDecision(intent=unknown)
    if has_file_reference(x) or has_code_structure_hint(x):
        return RoutingDecision(intent=coding)
    # Extract 8 boolean feature flags:
    has_action / has_strong_action / has_workspace_object /
    has_deliverable / has_path / has_command / has_issue / has_tech
    flags = extract_flags(x)
    if has_issue AND (obj | cmd | tech):
        return RoutingDecision(intent=coding)
    if has_action AND (obj | deliverable | path | cmd):
        return RoutingDecision(intent=coding)
    if has_strong_action AND (tech | deliverable):
        return RoutingDecision(intent=coding)
    return RoutingDecision(intent=chat)
\end{lstlisting}

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.75\textwidth]{keyword_gate_flowchart}
    \caption{关键词组合门控判定流程图。首先过滤空输入至 $i_{\text{unknown}}$，然后通过文件引用快速通道、错误信号组合、动作动词组合和强动作组合四条分支依次判定，所有分支未命中时回退至 $i_{\text{chat}}$。}
...
i_{\text{unknown}} & \text{if } x \text{ is blank} \\
...
\begin{itemize}
    \item \textbf{Tier 1（始终调用 LLM）}：包括 \texttt{workspace.write}、\texttt{workspace.delete}、\texttt{workspace.move} 和 \texttt{workspace.copy}。这些操作具有不可逆性或高风险性，需要 LLM 从用户指令中精确抽取文件路径、内容等关键参数。
...
\midrule
tab $\geq$ 3 $\land$ activewindow $\geq$ 1 & browsing & neutral \\
activewindow $\geq$ 1 $\land$ menubar $\geq$ 1 & using an application & neutral \\
folder $\geq$ 2 & managing files & neutral \\
address bar $\geq$ 1 $\land$ folder $\geq$ 1 & navigating files & neutral \\
...
视觉感知以 60 秒为周期的后台轮询运行，与文本路由并行。数据流为：Electron 截屏 $\rightarrow$ \texttt{VisionMonitor} 读取 $\rightarrow$ YOLOv8 推理 $\rightarrow$ 活动分析；再通过 \texttt{RoleplayAgent.emit\_vision\_quip()} 生成俏皮话推送至前端。当 \texttt{runtime\_tracker} 检测到用户处于活跃任务中时，视觉俏皮话生成会被挂起。
...
脚本执行失败后启动自动修复决策（6 节点），依次执行：\texttt{inspect\_failure\_node} 汇总失败信息，\texttt{eligibility\_node} 检查 LLM 可用性和修复预算，\texttt{qa\_node} 分析失败原因，\texttt{decision\_node} 决定是否修复，\texttt{compose\_feedback\_node} 构建反馈消息，最后 \texttt{repair\_codegen\_node} 生成修复脚本。修复流程支持循环，直到成功、预算耗尽或用户取消。
...
代码执行采用本地 Python 子进程（\texttt{subprocess.\-Popen(\-shell=False)}），并配有安全过滤机制。阻塞的 shell 解释器包括 cmd、powershell、sh、bash；阻塞的危险参数包括 rm、del、format、shutdown、taskkill 等。所有文件操作限定在工作空间目录内，含路径穿越保护和敏感文件排除（.env、credentials.json 等）。文件操作仅支持 UTF-8 编码的文本文件。
