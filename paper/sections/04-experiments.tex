...
\textbf{加权 F1（Weighted F1）}：
\[
\text{F1}_{\text{weighted}} = \sum_{c \in \mathcal{C}} w_c \cdot \text{F1}(c)
\]
其中 $w_c = N_c / N$，$\text{F1}(c) = 2 \cdot P_c \cdot R_c / (P_c + R_c)$，
$\mathcal{C} = \{\text{chat}, \text{coding}, \text{unknown}\}$
...
\begin{figure}[htbp]
    ...
    \begin{subfigure}[b]{0.3\textwidth}
        ...
        \caption{空闲屏幕}
    \end{subfigure}
    \caption{YOLOv8 在三种典型桌面场景下的 UI 元素检测结果。}
